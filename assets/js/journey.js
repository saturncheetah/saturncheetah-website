...
if(form){
  ...
  const show=index=>{active=Math.max(0,Math.min(steps.length-1,index));steps.forEach((step,i)=>step.hidden=i!==active);back.hidden=active===0;updateNextLabel();counter.textContent=`Step ${active+1} of ${steps.length}`;summary.hidden=active!==steps.length-1;if(policyNote)policyNote.hidden=active!==steps.length-1;if(!summary.hidden)updateSummary();form.dispatchEvent(new CustomEvent("saturn:journey-step",{bubbles:true,detail:{step:steps[active],index:active,total:steps.length}}))};
